 ...
 * changes and trigger corresponding controllers to update their cache with the
 * latest content.
 */
export class DocumentWatcher implements Disposable {
    ...
    private _tick() {
        ...
        for (const rock of this.registry.getWorkspaceRocks()) {
            const home = rock.home.path + '/';
            const keys = dirtyKeys.filter(x => x.startsWith(home));
            for (const k of keys) {
                const uri = dirties.get(k)!;
                rock.updateLiveFile(uri).then(() => {
                    // nop
                });
            }
        }
